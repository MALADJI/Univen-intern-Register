...
public class SuperAdminController {
    ...
    public ResponseEntity<?> getAllAdmins() {
        try {
            ...
            List<Map<String, Object>> adminList = admins.stream().map(admin -> {
                ...
                Optional<User> userOpt = userRepository.findByUsername(admin.getEmail());
    ...
    public ResponseEntity<?> sendAdminInvite(@RequestBody Map<String, String> body) {
        try {
            ...
            if (adminRepository.findByEmail(trimmedEmail).isPresent() ||
    ...
    public ResponseEntity<?> createAdmin(@RequestBody Map<String, Object> body) {
        try {
            ...
            if (userRepository.findByUsername(trimmedEmail).isPresent() ||
                    adminRepository.findByEmail(trimmedEmail).isPresent()) {
                return ResponseEntity.status(HttpStatus.CONFLICT)
                        .body(Map.of("error", "Admin with this email already exists"));
            }
            ...
            if (body.containsKey("departmentId")) {
                ...
                if (departmentId != null) {
                    ...
                    if (deptOpt.isPresent()) {
                        admin.setDepartment(deptOpt.get());
                        System.out.println(
                                "✓ Setting department to: " + deptOpt.get().getName() + " (ID: " + departmentId + ")");
                    } else {
                        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    ...
                    System.out.println("⚠ Explicitly removing department (user set to null)");
                }
            } else {
    ...
    public ResponseEntity<?> deleteAdmin(@PathVariable Long adminId) {
        try {
            ...
            Optional<User> userOpt = userRepository.findByUsername(adminEmail);
            if (userOpt.isPresent()) {
                User user = userOpt.get();
