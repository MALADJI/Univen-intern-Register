...
public class AuthController {
    ...
    private void createInternProfile(User user, Map<String, Object> request) {
        try {
            ...
            if (supervisor == null) {
                Optional<Supervisor> supervisorOpt = supervisorRepository.findFirstByDepartmentId(finalDeptId);
                if (supervisorOpt.isPresent()) {
                    supervisor = supervisorOpt.get();
    ...
    private void fetchDepartmentInfo(User.Role role, String email, Map<String, Object> userData) {
        try {
            if (role == User.Role.ADMIN) {
                ...
                Optional<Admin> adminOpt = adminRepository.findByEmailWithDepartment(email);
                ...
                Optional<Supervisor> supervisorOpt = supervisorRepository.findByEmailWithDepartment(email);
                if (supervisorOpt.isPresent()) {
                ...
                Optional<Intern> internOpt = internRepository.findByEmailWithDepartment(email);
