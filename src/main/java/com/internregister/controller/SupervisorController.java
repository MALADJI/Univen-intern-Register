...
public class SupervisorController {
    ...
    public ResponseEntity<?> getSupervisorByEmail(@PathVariable String email) {
        try {
            Optional<Supervisor> supervisorOpt = supervisorRepository.findByEmail(email.trim().toLowerCase());
            ...
            Optional<User> userOpt = userRepository.findByUsername(supervisor.getEmail());
    ...
    public ResponseEntity<?> createSupervisor(@RequestBody Map<String, Object> body) {
        try {
            ...
            if (supervisorRepository.findByEmail(trimmedEmail).isPresent() ||
                    userRepository.findByUsername(trimmedEmail).isPresent()) {
    ...
    public ResponseEntity<?> updateSupervisor(@PathVariable Long id, @RequestBody Map<String, Object> body) {
        try {
            ...
            if (body.containsKey("email")) {
                ...
                if (email != null && !email.trim().isEmpty()) {
                    ...
                    if (!trimmedEmail.equals(oldEmail)) {
                        if (userRepository.findByUsername(trimmedEmail).isPresent() ||
                                supervisorRepository.findByEmail(trimmedEmail).isPresent()) {
                            return ResponseEntity.status(HttpStatus.CONFLICT)
            ...
            Optional<User> userOpt = userRepository.findByUsername(updated.getEmail());
            if (userOpt.isPresent()) {
                User user = userOpt.get();
    ...
    public ResponseEntity<?> deleteSupervisor(@PathVariable Long id) {
        try {
            ...
            Optional<User> userOpt = userRepository.findByUsername(supervisorEmail);
            if (userOpt.isPresent()) {
                User user = userOpt.get();
